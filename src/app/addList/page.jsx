"use client"
import { addUser } from '@/lib/action';
import { useFormState } from 'react-dom';
import { Form, Button, Input, Select, TimePicker } from 'antd';

const page = () => {
  const onFinish = (values) => {
    console.log('Success:', values);
  }
  const roomOptions = [
    { value: '1', label: '变电站' },
    { value: '2', label: '配电室' },
    { value: '3', label: '锅炉房' },
  ]
  const personnelOptions = [
    { value: '1', label: '杨方伟' },
    { value: '2', label: '龚建波' },
    { value: '3', label: '王菲' },
  ]


  return <div>
    <h1>添加工单</h1>
    <Form onFinish={onFinish} labelCol={{span: 6}}>
      <Form.Item label="维修车间" name="room">
        <Select options={roomOptions} placeholder="请选择维修车间" />
      </Form.Item>
      <Form.Item label="维修起始时间" name="startTime">
        <TimePicker placeholder="维修起始时间" />
      </Form.Item>
      <Form.Item label="维修工时" name="manhour">
        <Input type="email" placeholder="请填写维修工时" />
      </Form.Item>
      <Form.Item label="维修内容" name="content">
        <Input.TextArea rows={4} placeholder="请填写维修内容" />
      </Form.Item>
      <Form.Item label="维修人员" name="personnel">
        <Select options={personnelOptions} placeholder="维修人员" />
      </Form.Item>
      <Form.Item label={null}>
        <Button type='primary' htmlType="submit">保存工单</Button>
      </Form.Item>
    </Form>
  </div>

}

export default page
